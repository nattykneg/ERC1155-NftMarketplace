...
; (function ($) {
    ...
    var buttonHeart = function () {
        $(".wishlist-button").on("click", async function () {
            if (window.ethereum && localStorage.getItem('account')) {
                ethereum.request({ method: 'eth_requestAccounts' })
                    .then((accounts) => {
                        ...
                        var nftName = nftItem.attr('name');
                        var nftId = nftName.slice(13);
                        ...
                        $.ajax({
                            ...
                            success: function (response) {
                                ...
                                if (data.status) {
                                    nftItem.addClass("active");
                                    var nftNameList = document.getElementsByName(nftName);
                                    for (let index = 0; index < nftNameList.length; index++) {
                                        nftNameList[index].classList.add("active")
                                        const element = nftNameList[index].children[0];
                                        var val = parseInt(element.textContent) + 1;
