    ...
    <div class="page-header-inner ">
     ...
     ?>
        <div class="page-top" data="bharat">
            ...
            @elseif($slug=='admin')
                <ul class="nav navbar-nav pull-right" style="display: flex; align-items: center;">
                    <li class="separator hide"> </li>
                    <li>
                        <div id="showAccount">
                            <a class="btn btn-primary" onclick="connectMetamask()">Connect Metamask</a>
                        </div>
                    <li>    
                    <li class="dropdown dropdown-user dropdown-dark">
                        <a href="javascript:;" class="dropdown-toggle" data-toggle="dropdown" data-hover="dropdown" data-close-others="true">
...
<script>
    async function connectMetamask() {
        if (window.ethereum) {
            ...
            buttonElement.innerHTML = accounts[0];
            document.getElementById('showAccount').innerHTML = '';
            document.getElementById('showAccount').append(buttonElement);
        } else {
    ...
    async function setUserProfile() {
        if (window.ethereum && window.ethereum.selectedAddress) {
            ...
            contract.methods.getAllBitsports().call(async function (err, nfts) {
                if (err) {
                    return;
                }
                ...
                for (let index = 0; index < nfts.length; index++) {
                    if (!nfts[index].forSale) {
                        if (nfts[index].currentOwner.toUpperCase() == accounts[0].toUpperCase() && nfts[index].isActive) {
                            var metadata = await $.ajax({ 
                                url: ('https://ipfs.io/ipfs/' + nfts[index].tokenURI),
                                dataType: "json"
                            });
                            
                            $('#userProfile').attr('src', 'https://ipfs.io/ipfs/' + metadata.imageUrl);
                        }
