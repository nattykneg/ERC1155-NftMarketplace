<script type="text/javascript">
    ...
    if(localStorage.getItem("betting")){
        ...
        $.each(olddstore_bet, function (i) {
            store_bet.push({
                bet_id: olddstore_bet[i].bet_id,
                odd_select: olddstore_bet[i].odd_select,
                odd_id: olddstore_bet[i].odd_id,
                more_id: olddstore_bet[i].more_id
            });
    ...
    function new_odds(elem){
        ...
        function(data, status){
            if(data>0){
                ...
                var check = 0;
                check = store_bet.filter(function (bet) {if (bet.bet_id == betid ) { return bet }  });
               if(check!=0){ 
    ...
    function mul_final_calculation(single_input){
        ...
        if(parseFloat(balance) < parseFloat(potential) ){ 
            $(".placeBtnMul").attr('disabled',true);
            $(".alert2").fadeIn("slow");
            disable =1;
        }else{
